...
## Follow-Up Rejected Experiment D: Trajectory-Aware Prototype Summary

Hypothesis:

- the current `5D` prototype summary is too close to an endpoint cluster
- adding a small amount of trajectory-shape information to the prototype summary may make prototype assignment more mode-aware
- if the prototype layer becomes more informative, later `FDE@5 / Top1_FDE` may improve without breaking the ProtoBasis story

Implementation tested:

- replace the current `5D` summary:
  - final local endpoint `xyz`
  - total `delta_yaw`
  - total `delta_pitch`
- with an `11D` summary:
  - final local endpoint `xyz`
  - local anchor at about `1/3` future horizon `xyz`
  - local anchor at about `2/3` future horizon `xyz`
  - total `delta_yaw`
  - total `delta_pitch`
...
- do **not** run a training comparison first
- first compare artifact-level diagnostics
- only continue into training if prototype-level `FDE` gets better rather than worse

Controlled diagnostic command:

```bash
python scripts/protobasis_artifact_diagnostic.py 111_days --limit 500
```

Baseline artifact diagnostic (`5D` summary):
...
Trajectory-aware summary diagnostic (`11D` summary):

- `summary_dim = 11`
- `rare_proto_count = 45`
- `proto_base_ADE_mean = 0.8133`
- `proto_base_FDE_mean = 0.6749`
- `recon_ADE_mean = 0.0765`
- `recon_FDE_mean = 0.6749`

Decision:

- the change slightly improved prototype-side `ADE`
- but the more important endpoint-facing quantities got worse:
  - `proto_base_FDE`
  - `recon_FDE`
- it also slightly worsened reconstruction-side `ADE`

Interpretation:

- simply injecting two intermediate anchors into the prototype summary does not solve the real bottleneck
- under the current artifact design, prototype grouping and endpoint anchoring are still tightly tied to endpoint structure
- this version makes clusters more shape-aware, but not in a way that improves endpoint quality

Conclusion:

- do not spend training budget on this version
- the artifact-level gate already says it is not the right prototype change in its current form
- the next prototype-side attempt should be more endpoint-aware and more structured, rather than just increasing summary dimensionality
