...
## 2026-04-21 Best20/GLeV Smoke Search

Goal:

- push `ADE@20` materially below the current smoke baseline while also improving late `GLeV_report@20`
- focus on paper-facing `best@20` metrics rather than `best@5`

Controlled proxy setup:

- dataset: `111_days`
- schedule: `10 / 4 / 12` with `extra_epochs=0`
- `batch_size=512`, `eval_batch_size=1024`
- `limit_train_batches=120`, `limit_eval_batches=20`
- same seed for all runs

Smoke baseline (`current mainline`):

- `ADE@20 = 0.3727`
- `FDE@20 = 0.4575`
- `rare_FDE@20 = 1.2063`
- final `GLeV_report@20 = 0.1707`

Rejected directions:

- `stage_c_score_weight=0, rank_weight=0`:
  - improved `ADE@20/FDE@20`
  - did not improve late `GLeV`
  - collapsed `best@5/Top1`
- full-trajectory soft geometry:
  - improved `GLeV`
  - degraded `ADE@20/FDE@20`

Promising direction kept for follow-up:

- keep `Stage C` score/rank disabled
- increase `Stage C` diversity pressure
- add endpoint-cover loss plus trajectory-cover loss as additive set-level objectives

Best proxy found in this search:

- args:
  - `--stage_c_score_weight 0.0`
  - `--stage_c_rank_weight 0.0`
  - `--stage_c_div_weight 5.0`
  - `--lambda_cover 0.5`
  - `--cover_temperature 0.35`
  - `--lambda_traj_cover 0.3`
...
- results:
  ...
  - `FDE@20 = 0.4433`
  - `rare_FDE@20 = 1.2236`
  - final `GLeV_report@20 = 0.1394`

Interpretation:

- relative to the smoke baseline:
  - `ADE@20` improves by about `16.5%`
  - late `GLeV_report@20` improves by about `18.3%`
  - `FDE@20` improves modestly
  - `rare_FDE@20` regresses slightly in smoke
- rough extrapolation using the current `100`-epoch run suggests:
  - `ADE@20` can plausibly approach `~0.20`
  - `GLeV_report@20` can plausibly approach `~0.057`
- this is the first smoke result in the search that simultaneously pushes precision and diversity toward the target region without requiring a backbone change
